...
public class FightResultsRepository : IFightResultsRepository
{
    ...
    public FightResultsRepository(string outputBasePath)
    {
    ...
    {
        if (!Directory.Exists($"{_outputBasePath}//{outputPath}"))
            Directory.CreateDirectory($"{_outputBasePath}//{outputPath}");
    ...
    public void FlushResults(string outputPath)
    {
        ...
        File.WriteAllText($"{_outputBasePath}//{outputPath}//bestResults.txt", output);
    ...
    public void SaveFighterResults(List<AttackResult> results, string outputPath)
    {
        ...
        foreach (var fighterResults in resultsByFighter)
        {
            ...
            File.WriteAllText($"{_outputBasePath}//{outputPath}//{fighterResults.Key}.results.txt", fighterResultsCsv);
        }
    }
    ...
    {
        if (!Directory.Exists($"{_outputBasePath}//{outputFolder}"))
            return null;

        var directoryInfo = new DirectoryInfo($"{_outputBasePath}//{outputFolder}");
