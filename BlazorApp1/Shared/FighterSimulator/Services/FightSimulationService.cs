...
public class FightSimulationService
{
    ...
    public AttackResult SimulateCityAttack(Func<Army, Army, Army> yourArmyFunc, Func<Army, Army, Army> enemyArmyFunc, FightSimulationOptions options)
    {
        ...
        while (yourArmy.MainPlayerIsAlive && enemyArmy.MainPlayerIsAlive && (enemyArmy.AnyPlayerIsAlive || !options.RunToAllPlayersDead))
        {
            ...
            {
                EnemyAttackDefenceRatio = enemyArmy.Troops.Average(x => x.CalculatedAttack) / yourArmy.Troops.Average(x => x.CalculatedDefence)
            };
            ...
            log.EnemyLostTroops = ProcessArmyLosses(enemyArmy, log.YourNormalDamage);
            
            // TODO: Implement different logic for counter attacks
            yourArmy.RageLevel += RageOnNormalAttack;
            
            
            var yourSkillsResult = ProcessSkills(yourArmy, enemyArmy, log.YourBaseDamage, options);
            ...
            {
                log.YourLostTroops = ProcessArmyLosses(yourArmy, log.EnemyNormalDamage);
                
                enemyArmy.RageLevel += RageOnNormalAttack;
                var defenderSkillsResult = ProcessSkills(yourArmy, yourArmy, log.EnemyBaseDamage, options);
            }
            ...
            log.YourTroopsRemaining = yourArmy.TotalTroopsCount;
            log.EnemyTroopsRemaining = enemyArmy.TotalTroopsCount;
